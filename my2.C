...
//by  b_branchname->GetEntry(ientry); //read only this branch
   TFile *f = new TFile(Form("hf%d.root", RunNo), "RECREATE");   

   TH1D *H = new TH1D(Form("H"), Form("H"), 2000, 0, 2000);
   TH2D *HH = new TH2D(Form("ha"),Form("hb"),1600,0,160,1600,0,160);
   ...
   //    if(channel[i] == cha1) {
   //       g1 = channel[i] * 0.002385;
   //       for(int j=0; j < nXIA ; j++) {
   //             if(channel[j] == cha2) {
   //                 g2 = channel[j];
   //                if(cut1 -> IsInside(channel[i] * 0.002385, channel[j])) {
   ...
   //       }
   // }
      for(int i=0; i < nXIA ; i++) if(channel[i] == cha1) 
         for(int j=0; j < nXIA ; j++) if(channel[j] == cha2) 
            if(cutttt -> IsInside(Energy[i] * 0.002385, Energy[j])) 
               for(int k=0; k < nXIA; k++)   if(channel[k] < 299 )
               {  
               // std::cout << "Condition met at index: " << i << std::endl;
                  HH->Fill(Energy[i]* 0.002385, Energy[j]);
                  H->Fill(Energy[k]);
               }
   }
